...
let constant f = function
  ...
  | Const_int i -> paren (i<0) (fun f -> pp f "%d") f i
  | Const_float  i -> paren (i.[0]='-') (fun f -> pp f "%s") f i
  | Const_int32 i -> paren (i<0l) (fun f -> pp f "%ldl") f i
  | Const_int64 i -> paren (i<0L) (fun f -> pp f "%LdL") f i
                                       (* pp f "%LdL" i *)
  | Const_nativeint i -> paren (i<0n) (fun f -> pp f "%ndn") f i
                                           (* pp f "%ndn" i *)
...
and type_with_label ctxt f (label,({ptyp_desc;_} as c)) =
  match label with
...
and sugar_expr ctxt f e =
  ...
  (* should also check attributes underneath *)
    ...
    | Pexp_apply
        ...
        pp f fmt (simple_expr ctxt) e1 (expression ctxt) e2;
        true
      end
    |Pexp_apply
        ({pexp_desc=
      ...
      ->
        ...
        pp f fmt (simple_expr ctxt) e1 expression e2 expression e3;
        true
    ...
    | Pexp_apply
        ({pexp_desc=Pexp_ident
              {txt= Ldot (Ldot (Lident "Bigarray", array),
                          ("get"|"set" as gs)) ;_};_},
         label_exprs) ->
        let simple_expr = simple_expr ctxt in
        begin match array, gs, label_exprs with
        | "Genarray", "get",
          [(_,a);(_,{pexp_desc=Pexp_array ls;_})] ->
            pp f "@[%a.{%a}@]" simple_expr a
              (list ~sep:"," simple_expr) ls;
            true
        | "Genarray", "set",
          [(_,a);(_,{pexp_desc=Pexp_array ls;_});(_,c)] ->
            pp f "@[%a.{%a}@ <-@ %a@]" simple_expr a
              (list ~sep:"," simple_expr) ls simple_expr c;
            true
...
and expression ctxt f x =
  if x.pexp_attributes <> [] then
    ...
    | Pexp_ifthenelse _ | Pexp_sequence _ when ctxt.ifthenelse ->
        paren true (expression reset_ctxt) f x
    | Pexp_let _ | Pexp_letmodule _ when ctxt.semi ->
        paren true (expression reset_ctxt) f x
    | Pexp_fun (l, e0, p, e) ->
        pp f "@[<2>fun@;%a@;->@;%a@]"
        ...
        begin if not (sugar_expr ctxt f x) then
            ...
            | `Prefix s ->
                ...
                | _   ->
                    (*FIXME assert false*)
                    pp f "@[<2>%s@;%a@]"
                      s (list (label_x_expression_param ctxt)) l
                end
            | _ ->
...
and binding ctxt f {pvb_pat=p; pvb_expr=x; _} =
  ...
  then pp f "%a@;=@;%a" (pattern ctxt) p (expression ctxt) x
    ...
    | Pexp_constraint (e,t1),Ppat_var {txt;_} ->
        pp f "%s:@ %a@;=@;%a" txt (core_type ctxt) t1 (expression ctxt) e
    | (_, Ppat_var _) ->
...
and type_declaration ctxt f x =
  ...
  let manifest f =
    ...
    | None -> ()
    | Some y -> pp f "@;%a" (core_type ctxt) y
  in
  ...
  let repr f =
    ...
    | Ptype_abstract -> ()
    | Ptype_record l ->
        pp f "%t@;%a" intro (record_declaration ctxt) l
...
and toplevel_phrase ctxt f x =
  match x with
  | Ptop_def (s) ->
      pp_open_hvbox f 0;
      list (structure_item ctxt) f s;
      pp_close_box f ();
  | Ptop_dir (s, da) ->
